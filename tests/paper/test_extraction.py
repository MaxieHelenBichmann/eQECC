...
def _stat_row(
    ...
    *,
    ...
    return {
        ...
        "num_cases": nr_seeds,
        "num_successful": nr_seeds if successful is None else successful,
        "num_unexpected": 0,
...
def test_read_statistics_keeps_latest_same_sized_invocation(tmp_path: Path) -> None:
    ...
    write_csv(
        path,
        [_stat_row("pm_stb_sat", True, 1.0, nr_seeds=3), _stat_row("pm_stb_sat", True, 2.0, nr_seeds=3)],
        CSV_FIELDS,
    )
...
def test_changed_seed_count_supersedes(tmp_path: Path) -> None:
    ...
    write_csv(
        path,
        [_stat_row("pm_stb_sat", positive, nr_seeds=nr_seeds) for nr_seeds in (3, 5) for positive in (True, False)],
        CSV_FIELDS,
    )
...
def test_a6_uses_two_complete_files_and_only_the_extra_css_file(tmp_path: Path) -> None:
    ...
    write_csv(
        extra, [_stat_row("pm_stb_sat_on_css", True, 10.0), _stat_row("pm_stb_sat_on_css", False, 10.0)], CSV_FIELDS
    )
...
def test_a8_aggregates_runtimes_and_deciding_stages_per_cell(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    write_csv(
        tmp_path / "pm_stb_raw.csv",
        [
    ...
    assert [(cell["problem"], cell["code"], cell["positive"]) for cell in cells] == [
        ("pm_stb", "bell", True),
        ("pm_stb", "bell", False),
    ]
